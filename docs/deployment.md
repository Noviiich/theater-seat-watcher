...
Backup при деплое выполняет `deploy/backup-database.py`: workflow передаёт его
через stdin в Python уже работающего контейнера. Скрипт использует только
стандартную библиотеку, SQLite Backup API и проверку целостности копии;
загрузка runtime, aiogram и SQLAlchemy не требуется. Это уменьшает расход
памяти рядом с ботом на сервере с 1 ГБ RAM и работает даже со старым образом.
Копия публикуется атомарно с правами `0600`, существующие файлы не заменяются.
При ошибке backup скрипт не создаёт маркер готовности и не переходит к остановке
бота или удалению образа.

Код 137 на этом шаге соответствует завершению через SIGKILL; нехватка памяти
возможна, но сам код не доказывает OOM. На сервере проверьте журнал ядра за время
сбоя (`sudo journalctl -k --since '2026-09-26 16:00:00 UTC'`), сообщения
`Out of memory` / `Killed process`. Повторять нужно workflow с исправленным
коммитом: повтор старого job использует прежний скрипт.
