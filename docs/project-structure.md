...
Сейчас существуют `AGENTS.md` и `docs/`. Ниже — **целевая** структура,
которая создаётся постепенно по roadmap; пустые модули заранее не нужны.
...
│   │   ├── booking.py
│   │   ├── renewals.py             # Сроки циклов, stop/paid, повтор после 180 секунд
│   │   └── reconciliation.py
│   ├── adapters/
│   │   ├── quicktickets/
...
позволяет проверить границы 179/180/181 секунд и перезапуск без реального ожидания.
