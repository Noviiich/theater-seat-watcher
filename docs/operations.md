...
- `POLL_INTERVAL_SECONDS=2.5`, `POLL_JITTER_SECONDS=0` — начало проверок афиши
  через 2,5 секунды при достаточно быстром проходе; работающий процесс читает
  изменения `.env` только после перезапуска. Долгая полная сверка и ошибки
  провайдера увеличивают фактический интервал.
...
В структурированных логах события `catalogue_read` и `catalogue_discovery`
показывают время чтения афиши, загрузки деталей и записи кандидатов. События
`seat_evaluation`, `checkout_submit`, `order_recorded` и `notification_sent`
показывают длительность этапов от подбора мест до отправки ссылки; в них нет
данных покупателя, содержимого заказа и платёжного URL.
