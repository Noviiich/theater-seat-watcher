...
оформляется сразу; затем бот получает **новую бронь и ссылку каждые 3 минуты**
при наличии удобных соседних мест. `RenewalDue` запускает следующий цикл после
подтверждённого истечения неоплаченного заказа. Оплата/остановка завершают повторы.
...
| --- | --- |
| `Buyer` | Telegram user/chat ID, ссылка на защищённый профиль покупателя, общие лимиты |
...
После окончательного неоплаченного истечения автоматически создаётся следующий
RenewalCycle; после оплаты/явной отмены повторов нет. Технический retry после
...
## Планировщик трёхминутных циклов

Три независимых расписания: афиша — 60 секунд с jitter; ожидание свободных мест
— 180 секунд; повторная бронь — `renewal_interval_seconds=180` от `held_at`.
TTL 180 секунд сообщён пользователем. Берём фактический `expires_at` из checkout
или вычисляем его по проверенному протоколу отсчёта, не по времени доставки.

```text
t=0:      hold цикла 1 → немедленная доставка ссылки 1
t≈180s:   проверить исход 1 → перечитать места → hold 2 → ссылка 2
t≈360s:   проверить исход 2 → перечитать места → hold 3 → ссылка 3
paid:     прекратить повторы только этого сеанса
```

Следующий checkout разрешён не раньше `max(held_at + 180s, expires_at)` и только
после окончательного `expired_unpaid`. Если подтверждение/освобождение позже,
оформление сдвигается; новый `held_at` становится точкой отсчёта следующего цикла.
Если фактический TTL короче, ссылку сразу считаем истёкшей по нему, а до 180 секунд
ждём; если длиннее, раньше времени активную бронь не заменяем.

`expired_unpaid` — внутренний подтверждённый результат: заказ не оплачен,
нет незавершённого платежа и старую ссылку больше нельзя оплатить. Это не
придуманный статус QuickTickets; его соответствие внешнему контракту должен
установить адаптер. Свободные кресла и локальный таймер сами по себе его не доказывают.
Промежуточный `processing` или неизвестный исход переводит в reconcile.

После завершения цикла в одной транзакции обновляются статус Order, завершение
RenewalCycle, освобождение allocation и `Candidate.next_run_at`. Новый цикл
...
Планировщик читает сохранённые `next_run_at`, а не держит отдельный трёхминутный
sleep на каждый сеанс. На старте сначала reconcile последних заказов; пропущенные
интервалы схлопываются до одной актуальной задачи. Счётчики и время не сбрасываются.
`/stop`, пауза и истечение watch_until отключают новые checkout, но сохраняют
reconcile/outbox текущего заказа. После `/resume` сначала проверяется его исход.
...
Денежный лимит пакета включает оплаченные заказы и активные allocations;
неопределённые операции учитываются по зарезервированному потолку. Активные
лимиты покупателя освобождаются только при подтверждённом терминальном статусе.
Истёкшие неоплаченные заказы остаются в истории, но освобождают allocation:
два последовательных цикла по 2400 ₽ требуют 2400 ₽ доступного бюджета,
а не 4800 ₽. При повторе с новой ценой выполняется новая проверка всех лимитов.
...
stateDiagram-v2
    ...
    awaiting_payment --> paid: подтверждено продавцом
    awaiting_payment --> verifying_expiry: наступил срок удержания
    verifying_expiry --> paid: платёж подтверждён
    verifying_expiry --> awaiting_payment: провайдер подтвердил более поздний срок
    verifying_expiry --> cancelled: подтверждена явная отмена
    verifying_expiry --> reconciling: платёж в обработке или исход неизвестен
    verifying_expiry --> renewal_waiting: окончательно expired_unpaid
    reconciling --> renewal_waiting: окончательно expired_unpaid
    reconciling --> paid: платёж подтверждён
    reconciling --> cancelled: подтверждена явная отмена
    renewal_waiting --> queued: новый цикл в срок и в лимитах
    renewal_waiting --> stopped: stop или watch_until или лимит циклов
    awaiting_payment --> cancelled: подтверждено продавцом
```

`needs_attention` сохраняет allocation и блокирует новое оформление этого
Candidate до разрешения ситуации. Local timeout после `expires_at` помечает
ссылку как предположительно истёкшую и запускает проверку; без API статуса нельзя
выдумывать терминальное состояние. Ручное разрешение сохраняет основание в истории.
Диаграмма отражает обработку Candidate, а не перезапись старого Order:
терминальные статусы старых заказов сохраняются. Отмена брони поддерживается
только при наличии проверенного пути отмены и завершает повторы. Кнопка остановки
блокирует новые циклы без требования отмены текущего заказа. Stop/pause и
watch_until проверяются перед каждым новым checkout, даже если переход
не изображён на диаграмме.
...
провайдера. Плановое новое оформление после expired_unpaid — отдельный цикл.
...
- Конфигурация: `TELEGRAM_BOT_TOKEN`, `ALLOWED_TELEGRAM_USER_IDS`, `DATABASE_URL`,
  ...
  `RENEWAL_INTERVAL_SECONDS=180`, `EXPECTED_HOLD_TTL_SECONDS=180`,
