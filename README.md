...
оформляет билеты и отправляет подтверждённые ссылки на оплату.
Новый [режим организации](docs/organization-booking.md): до 70 билетов на новый
сеанс, строго одно место на заказ и отдельная ссылка на каждый билет,
без предпочтений по соседству и цене, с соблюдением ограничений продавца.
...
write transport отсутствует. Live требует подтверждения новой подписки:
в прежнем режиме задаются денежные пределы, в режиме организации явно
